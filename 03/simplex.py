...
def canonical_simplex(simplex_matrix, Q, P, x0, flags, for_two_phase=False):
    ...
    while True:
        ...
        if (pure_f[:-1] >= 0).all():
            if not flags['canon']:
                if for_two_phase:
                    return np.around(x0, DEC)[:-simplex_m + 1], np.round(np.dot(x0, c[:-1]), DEC), P
                else:
                    return np.around(x0, DEC)[:-simplex_m + 1], np.round(np.dot(x0, c[:-1]), DEC)
            else:
                if for_two_phase:
                    return np.around(x0, DEC), np.round(np.dot(x0, c[:-1]), DEC), P
                else:
